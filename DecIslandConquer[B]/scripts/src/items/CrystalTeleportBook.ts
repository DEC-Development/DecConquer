import { Dimension, Entity, Player, ScoreboardObjective, world } from "@minecraft/server";
...
function get_random_equipment(t: ModalFormResponse, player: Player) {
    let whether_queue = ConquerSettings.getSettings()['equipment_queue']
    ...
    if (t.formValues![1] == 0) {
        ...
        if (whether_queue) {
            ...
            equipment_queue.give(player,Career.Warrior)
        } else {
            mult_run_command(player, [
...
export class crystal_teleport_book {
    constructor() { }
    static crystal_teleport_book_index(player: Player) {
        if (get_score('global', 'game_state') == 0) {
            ...
            index.show(player).then(t => {
                let ob_type = manage_body[(<Array<number>>t.formValues)![0]].slice(0, 7)
