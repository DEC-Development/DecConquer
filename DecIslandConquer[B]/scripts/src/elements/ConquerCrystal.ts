...
export class ConquerCrystal {
    ...
    static occupyStatistic(cc:Entity) {
        ...
        const nearby_players = <Array<Player>>cc.dimension.getEntities(occupyStatisticSetOptions)
        let occ_weight = 0
        let d_score = 0
        ...
        if (nearby_players.length > 0) {
            ...
            if (occ_weight > 0 && cur_score != 100) {
                ...
                let p_location = <Vector3>cc.location
                p_location.y += 1.3
        ...
        //Vision effect
        let crys_particle_location = <Vector3>cc.location
