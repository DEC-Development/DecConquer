import { EntityQueryOptions, world, Player, GameMode } from "@minecraft/server"
import { if_in, mult_run_command, in_range, get_score, game_end } from "../func"
import { PlayerScore } from "./PlayerScore"
import { MathUtil } from "./MathUtil"

export class Team {
    static readonly redPlayerOption: EntityQueryOptions = {
        tags: ['team_red']
    }
    static readonly bluePlayerOption: EntityQueryOptions = {
        tags: ['team_blue']
    }
    static readonly redCrysOptions: EntityQueryOptions = {
        tags: ['team_red'],
        type: 'dec:conquer_crystal'
    }
    static readonly blueCrysOptions: EntityQueryOptions = {
        tags: ['team_blue'],
        type: 'dec:conquer_crystal'
    }
    static readonly allCrysOptions: EntityQueryOptions = {
        type: 'dec:conquer_crystal'
    }
    constructor() { }
    ...
    static playerStatistics(p: Player) {
        let player_with_team_arr = new Array<string>
        let red_t = 0
        let blue_t = 0
        let no_t = 0
    ...
    static scoreStatistics(p: Player) {
        let c_msg = ''
        let blue_oc = 0
        let red_oc = 0
        let no_oc = 0
    ...
    static autoTeamChooseHalfway(p: Player) {
    ...
    static autoTeamChoose() {
        ...
        players.sort((a, b) => (<number|undefined>a.getDynamicProperty('conquer_score') ?? 0) - (<number|undefined>b.getDynamicProperty('conquer_score') ?? 0))//From the smallest to the largest
    ...
    static scoreList(set_to_player = false) {
        let txt = ''
        let player_score_list = new Array<PlayerScore>
