...
world.afterEvents.itemUse.subscribe(e => {
    ...
    if (e.itemStack.typeId == 'dec:crystal_manage_book') {
        ...
        if (e.source.isSneaking) {
            ...
            Team.scoreStatistics(<Player>e.source)
        }
...
world.afterEvents.entityDie.subscribe(e => {
    if (e.deadEntity.typeId == 'minecraft:player') {
        ...
        if (if_in('gamer', e.deadEntity.getTags())) {
            ...
            if (de != undefined && de.typeId == 'minecraft:player' && if_in('gamer',de?.getTags())) {
                let hide_minus_board = world.scoreboard.getObjective('hide_minus_board')
                mult_run_command(de,[
                    'scoreboard players add @s kill_board 1',
                    'scoreboard players add @s hide_minus_board ' + Math.max(0,(hide_minus_board?.getScore(e.deadEntity.scoreboardIdentity ?? '') ?? 0) - (hide_minus_board?.getScore(de.scoreboardIdentity ?? '') ?? 0))
                ])
            }
